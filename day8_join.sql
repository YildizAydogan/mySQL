...
/* -----------------------------------------------------------------------------
  ...
  siparis_id ve siparis_tarihleri listeleyen bir sorgu yaziniz.
------------------------------------------------------------------------------*/ 
select  sir.sirket_isim, 
  sip.siparis_id, sip.siparis_tarihi
from sirketler sir
left join siparisler sip
on sir.sirket_id=sip.sirket_id
union
...
FROM
    sirketler AS sir
        LEFT JOIN
    siparisler AS sip ON sir.sirket_id = sip.sirket_id 
UNION SELECT 
    sir.sirket_id,
    sir.sirket_isim,
    sip.siparis_id,
...
select b.bolum_isim, p.personel_isim, p.maas, (select personel_isim from personel
                                        where p.mudur_id=personel_id) personel_muduru
